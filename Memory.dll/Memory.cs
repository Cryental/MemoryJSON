...
namespace Mem
{
    ...
    public class Mem
    {
        ...
        {
            return pattern.Split(' ').Select(each => each == "??" ? Convert.ToByte("0", 16) : Convert.ToByte(each, 16)).ToArray();
        }
        ...
        public int GetProcIdFromName(string name)
        {
            ...
            return (from preprocess in processlist where preprocess.ProcessName.Equals(name, StringComparison.CurrentCultureIgnoreCase) select preprocess.Id).FirstOrDefault();
        }
        ...
        {
            var sb = new StringBuilder();
            foreach (var c in str.Where(c => c >= ' ' && c <= '~'))
            {
                sb.Append(c);
            }
        ...
        public T Read<T>(long address)
        {
            ...
            ReadProcessMemory(pHandle, address, buffer, (uint)buffer.Length, out _);
        ...
        public int ReadInt(string code, string file = "")
        {
            var memory = new byte[4];
            var theCode = GetCode(code, file);
            return ReadProcessMemory(pHandle, theCode, memory, (UIntPtr)4, IntPtr.Zero) ? BitConverter.ToInt32(memory, 0) : 0;
        }
        ...
        public long ReadLong(string code, string file = "")
        {
            ...
            return ReadProcessMemory(pHandle, theCode, memory, (UIntPtr)16, IntPtr.Zero) ? BitConverter.ToInt64(memory, 0) : 0;
        }
        ...
        public ulong ReadUInt(string code, string file = "")
        {
            ...
            return ReadProcessMemory(pHandle, theCode, memory, (UIntPtr)4, IntPtr.Zero) ? BitConverter.ToUInt64(memory, 0) : (ulong) 0;
        }
        ...
        public int Read2ByteMove(string code, int moveQty, string file = "")
        {
            ...
            return ReadProcessMemory(pHandle, newCode, memory, (UIntPtr)2, IntPtr.Zero) ? BitConverter.ToInt32(memory, 0) : 0;
        }
        ...
        public int ReadIntMove(string code, int moveQty, string file = "")
        {
            ...
            return ReadProcessMemory(pHandle, newCode, memory, (UIntPtr)4, IntPtr.Zero) ? BitConverter.ToInt32(memory, 0) : 0;
        }
        ...
        public ulong ReadUIntMove(string code, int moveQty, string file = "")
        {
            ...
            return ReadProcessMemory(pHandle, newCode, memory, (UIntPtr)8, IntPtr.Zero) ? BitConverter.ToUInt64(memory, 0) : (ulong) 0;
        }
        ...
        public int Read2Byte(string code, string file = "")
        {
            ...
            return ReadProcessMemory(pHandle, theCode, memoryTiny, (UIntPtr)2, IntPtr.Zero) ? BitConverter.ToInt32(memoryTiny, 0) : 0;
        }
        ...
        {
            var memory = new byte[4];
            return ReadProcessMemory(pHandle, address + LoadIntCode(code, file), memory, (UIntPtr)1, IntPtr.Zero) ? BitConverter.ToInt32(memory, 0) : 0;
        }

        public float ReadPFloat(UIntPtr address, string code, string file = "")
        {
            var memory = new byte[4];
            if (!ReadProcessMemory(pHandle, address + LoadIntCode(code, file), memory, (UIntPtr) 4, IntPtr.Zero)) return 0;
        ...
        public string ReadPString(UIntPtr address, string code, string file = "")
        {
            ...
            return ReadProcessMemory(pHandle, address + LoadIntCode(code, file), memoryNormal, (UIntPtr)32, IntPtr.Zero) ? CutString(Encoding.ASCII.GetString(memoryNormal)) : "";
        }
        ...
        public bool Write<T>(long address, T t)
        {
            ...
            VirtualProtectEx(pHandle, (IntPtr)address, (UIntPtr)buffer.Length, 0x00000004, out var oldProtect);

            return WriteProcessMemory(pHandle, (UIntPtr)address, buffer, (UIntPtr)buffer.Length, out var ptrBytesWritten);
        }
        ...
        public UIntPtr GetCode(string name, string path = "", int size = 8)
        {
            ...
            if (newOffsets.Contains(','))
            {
                ...
                {
                    ReadProcessMemory(pHandle, (UIntPtr)((int)_mainModule.BaseAddress + offsets[0]), memoryAddress,
...

            {
                ...
                {
                    altModule = _mainModule.BaseAddress;
                }
                else
                {
                    if (!theCode.ToLower().Contains("base") && !theCode.ToLower().Contains("main") &&
        ...
        public UIntPtr Get64BitCode(string name, string path = "", int size = 16)
        {
            ...
            if (newOffsets.Contains(','))
            {
                ...
                if (theCode.Contains("base") || theCode.Contains("main"))
                {
                    ReadProcessMemory(pHandle, (UIntPtr)((long)_mainModule.BaseAddress + offsets[0]), memoryAddress,
                        (UIntPtr)size, IntPtr.Zero);
                }
                else
                {
            ...
            string file = "")
        {
            ...
            {
                caveAddress = VirtualAllocEx(pHandle, FindFreeBlockForRegion(preferred, (uint)size), (uint)size,
        ...
        private UIntPtr FindFreeBlockForRegion(UIntPtr baseAddress, uint size)
        {
            ...
            while (VirtualQueryEx(pHandle, current, out var mbi).ToUInt64() != 0)
            {
